...
class Producto {
  ...
  function productos_pagina() {
    base_de_datos.map((item) => {
      ...
      boton_violeta.setAttribute(
        "class",
        "btn violeta rounded-pill boton_añadir"
      );
  ...
  function abrir_carrito(array_carrito) {
    // Vacio el carrito
    itemsCarrito.textContent = "";
    console.log(array_carrito);//me trae la posicion del array carrito;
    array_carrito.forEach((item) => {
      let productoActual = base_de_datos.find(
        (elemento) => elemento.id == Number(item) //por cada item de mi array carrito busco el elemento de mi base de datos y comparo la posicion del array carrito vs su ID. Cuando sean iguales lo imprimo en consola
      );
      console.log(productoActual);

      let producto_sumado = document.createElement("div");
      producto_sumado.setAttribute(
        "class",
        "producto_sumado d-flex align-items-center my-2"
      );
      ...
      let btn_tacho = document.createElement('button');
      producto_sumado.appendChild(btn_tacho);
      ...
      tacho.setAttribute("tacho", productoActual.id); //para que cada boton de cada producto tenga un ID
      btn_tacho.appendChild(tacho);
    });
  }
